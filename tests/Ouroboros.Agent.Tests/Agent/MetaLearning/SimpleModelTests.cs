...
public class SimpleModelTests
{
    ...
    {
        // Arrange — model predicts output = input * a + b, target is a=2.5, b=0
        var model = new SimpleModel(
            LinearPrediction,
            new Dictionary<string, object>
            {
        ...
        // Act — compute gradients and update parameters
        var gradients = await model.ComputeGradientsAsync(examples);
        await model.UpdateParametersAsync(gradients, learningRate: 0.1);
    ...
    public async Task ComputeGradients_ArrayParameters_ElementWiseGradients()
    {
        ...
        // Each element should have a computed gradient — they should not all be identical
        // (different positions in the array may have same gradient due to symmetry in sum,
        // but at minimum they should be finite non-zero values)
        ...
        var distinctGradients = weightGrads.Distinct().Count();
