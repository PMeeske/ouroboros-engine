...
#pragma warning restore CA1031
            {
                logger?.LogWarning("[SFaceOnnx] DirectML unavailable — using CPU EP ({Reason})", ex.Message);
            }
        }
        else
        {
            logger?.LogWarning("[SFaceOnnx] DirectML execution provider not available — using CPU EP");
        }
